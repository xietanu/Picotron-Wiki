...
## Overview
For userdatas with a height value, returns `count` sequential numbers from the userdata starting at a specific column and row in flat index order. If the starting index is outside the indexable range, will return 0.0.
...
## Overview
For userdatas without a height value, returns `count` sequential numbers from the userdata starting at the index `i`. If the starting index is outside the indexable range, will return 0.0.
